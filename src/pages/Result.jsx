...
class Result extends Component {
  constructor({ match }) {
    ...
    if (previousResultId !== resultId) scrollTop = 0;
  ...
  componentWillUnmount() {
    if (this.scrollContainer) scrollTop = this.scrollContainer.scrollTop;
  }
  ...
  renderResult() {
    const { results, match, book, LANG, lang } = this.props;
    const resultId = match.params.resultId;
    const result = results[resultId];
    moment.locale(momentLangMap[lang]);
    return (
      <div className="ks-card-header-pic" ref={(ref) => { this.scrollContainer = ref; }}>
  ...
  render() {
    const { isAppReady, isFetchingResults, results, match, LANG } = this.props;
    const resultId = match.params.resultId;
    const result = results[resultId];
